...
const DriversTable = ({ type = "" }) => {
  ...
  const onDropDownListSuccess = (data) => {
    ...
    if (type === "manageDrivers") {
      setDriverDropDownList({
        ...
        email: [...new Set(data?.data?.dropDownDriverEmail?.map((item) => item.email))].map(
          (email) => ({ email: email })
        ),
        driver_type: [...new Set(data?.data?.dropDownDriverType?.map((item) => item.driver_type))].map(
          (driver_type) => ({ driver_type: driver_type })
        ),
        driving_license_id: [...new Set(data?.data?.dropDownDL?.map((item) => item.driving_license_id))].map(
          (driving_license_id) => ({ driving_license_id: driving_license_id })
        ),
      ...
      setDriverDropDownList({
        ...
        ].map((blocked_at) => ({ blocked_at: moment(blocked_at).format('DD-MM-YYYY') })),
      });
    } else if (type === "deletedDrivers") {
      // console.log(data?.data?.dropDownDriverId, "askjfhak");
      setDriverDropDownList({
        ...
        email: [...new Set(data?.data?.dropDownDL?.map((item) => item.email))].map(
          (email) => ({ email: email })
        ),
        first_name: [
          ...new Set(data?.data?.dropDownDriverFirstName?.map((item) => item.first_name)),
        ...
        ].map((deleted_at) => ({ deleted_at: moment(deleted_at).format('DD-MM-YYYY') })),
        driver_type: [...new Set(data?.data?.dropDownDriverType?.map((item) => item.driver_type))].map(
          (driver_type) => ({ driver_type: driver_type })
        ),
        driving_license_id: [...new Set(data?.data?.dropDownDL?.map((item) => item.driving_license_id))].map(
        ...
        registered_zone: [...new Set(data?.data?.dropDownZone?.map((item) => item.registered_zone))].map(
          (registered_zone) => ({ registered_zone: registered_zone })
        ),
      });
    } else if (type === "permanentlyDeletedDrivers") {
        ...
        permanently_deleted_by: [
          ...new Set(
            data?.data?.dropDownDriverPermanentlyDeletedby
              ...
              ?.filter((permanently_deleted_by) => Boolean(permanently_deleted_by))
          ),
        ].map((permanently_deleted_by) => ({ permanently_deleted_by: permanently_deleted_by })),
        permanently_deleted_on: [
          ...new Set(
            data?.data?.dropDownDriverDeletedAt
              ?.map((item) => item?.permanently_deleted_on)
              ?.filter((permanently_deleted_on) => Boolean(permanently_deleted_on))
          ),
        ].map((permanently_deleted_on) => ({ permanently_deleted_on: permanently_deleted_on })),
        driver_type: [...new Set(data?.data?.dropDownDriverAccounttype?.map((item) => item.driver_type))].map(
          (driver_type) => ({ driver_type: driver_type })
        ),
      ...
      setDriverDropDownList({
        driver_id2: [
          ...new Set(data?.data?.dropDownDriverId?.map((item) => item.driver_id2)),
        ].map((driver_id2) => ({ driver_id2: driver_id2 })),
        driving_license_id: [
          ...new Set(data?.data?.dropDownDL?.map((item) => item.driving_license_id)),
        ].map((driving_license_id) => ({ driving_license_id: driving_license_id })),
        first_name: [
          ...new Set(data?.data?.dropDownDriverFirstName?.map((item) => item.first_name)),
        ].map((first_name) => ({ first_name: first_name })),
        ...
        banned_at: [...new Set(data?.data?.dropDownDriverBannedAt?.map((item) => item.banned_at))].map(
          (banned_at) => ({ banned_at: banned_at })
        ),
      });
    }
    else if (type === "expiredDocuments") {
      ...
      // console.log(data?.data?.dropDownDriverId, "ddddd")
      setDriverDropDownList({
      ...
      //   ),
        ...
        ].map((rejected_at) => ({ rejected_at: moment(rejected_at).format('DD-MM-YYYY') })),
      });
    }
  };
  ...
  // console.log(driverDropDownList, "ddddd");
          ...
          <table className="table">
            <thead>
              <tr className="pale_blue_bg">
                <th scope="col" className="first_list">
                  <input
                    type="checkbox"
                  ...
                  >
                    ...
                    <td className="">
                      <span className="d-flex">
                        <NavLink
                          className="border_none border_radius fs_13 me-4 text-decoration-none fw_500 px-3 white_color blue_color_bg view_text"
                        ...
                        canWrite(permission) === false ? (
                          ...
                          <NavLink
                            className="border_none border_radius text-decoration-none primary_bg white_color fs_13 px-3 view_text"
                            to={`driver-rideHistory/${item?.id}`}
                            state={{
                              id: item?.id,
                              edit: true,
