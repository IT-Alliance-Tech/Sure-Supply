...
import { faresSpecialZoneListAction } from "../../../../../redux/actions/manageFaresAction";
...
import { archivedSpecialfaresListAction } from "../../../../../redux/actions/archiveFaresAction";

const FaresSpecialZonesTable = ({ paramsId, location }) => {
  ...
  useEffect(() => {
    setLoading(true);
    {
      is_archived_fare
        ? dispatch(
            archivedSpecialfaresListAction(
              {
  ...
  const onSuccess = (data) => {
    setSpecialZoneList(data?.data?.data);
    setPageData({
  ...
  const { items, requestSort, sortConfig } = useSortableData(SpecialZoneList);
  return (
    <>
        ...
        <table className="pale_blue_bg manage_fares_local_zones_table_container w-100 text-nowrap table">
          <thead className="w-100  pale_blue_bg">
            <tr className="text-start  fs_14 border-none  text-nowrap primary_color fw_600">
              <th className="text-nowrap ps-3" scope="col">
                Special Zone
              </th>
          ...
          <tbody className="light_blue_bg">
            <LoadAndError
              loader={loading}
              error={error}
              status={SpecialZoneList.length === 0}
            >
              {items?.map((item) => {
                return (
                  <tr key={item?.id} className=" secondary_color fw_500 ">
                    ...
                    <td className="pb-2 pt-2 ps-4">
                      <button
                        className="fs_14 blue_color_bg  border_radius view_btn  px-3 white_color border_none view_text"
                        onClick={() => {
                          navigate(
                            is_archived_fare
                              ? `/archived-special-fares/view/${paramsId[0]}&${paramsId[1]}&${item?.id}`
...
export default FaresSpecialZonesTable;
